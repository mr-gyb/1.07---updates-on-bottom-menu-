...
const NewChat: React.FC = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.value.trim() && currentChatId) {
      addMessage(currentChatId, 'user', e.target.value);
      // Simulate AI response
      setTimeout(() => {
        addMessage(currentChatId, 'assistant', `This is a response from ${selectedAgent} to your message: "${e.target.value}"`);
      }, 1000);
    }
  };
